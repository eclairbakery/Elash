...
void el_ast_dump_toplevel(ElAstTopLevelNode* node, usize indent, FILE* out) {
    ...
    case EL_AST_TOPLVL_FUNC_DEF:
        ...
        fprintf(out, "FuncDef(\""EL_SV_FMT"\", ret_type=\""EL_SV_FMT"\"):\n",
            EL_SV_FARG(node->as.func_def.name->name),
            EL_SV_FARG(node->as.func_def.ret_type->name));
        ...
        for (ElAstFuncParam* param = node->as.func_def.params.head; param; param = param->next) {
            ...
            fprintf(out, "Param(\""EL_SV_FMT"\", type=\""EL_SV_FMT"\")\n",
                EL_SV_FARG(param->name->name),
                EL_SV_FARG(param->type->name));
        }
